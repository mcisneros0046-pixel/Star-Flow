...
// In your Python version, data lived in workout_data.json.
// On the web, we use localStorage — same idea, built into every browser.
// Data stays on the user's device even after closing the browser.

function loadData() {
  try {
    const raw = localStorage.getItem("star-flow-data");
    if (raw) return JSON.parse(raw);
  } catch { /* first visit or corrupted data */ }
  return { entries: [], claimed: [] };
}

function saveData(data) {
  try {
    localStorage.setItem("star-flow-data", JSON.stringify(data));
...
function weekPts(entries, y, m, wn) {
  ...
  while (cur <= e) {
    total += dailyPts(entries, cur.toISOString().slice(0, 10));
...
function topTier(pts) {
  if (pts >= 15) return "gold";
  if (pts >= 12) return "silver";
  if (pts >= WEEKLY_TARGET) return "bronze";
  return null;
}
function availTiers(pts) {
  const t = [];
  if (pts >= 9) t.push("bronze");
  if (pts >= 12) t.push("silver");
  if (pts >= 15) t.push("gold");
  return t;
}
...
function fridayOf(y, m, wn) {
  const [s, e] = weekRange(y, m, wn);
  const cur = new Date(s);
  while (cur <= e) { if (cur.getDay() === 5) return cur; cur.setDate(cur.getDate() + 1); }
  return e;
}
function sundayOf(y, m, wn) {
  const [s, e] = weekRange(y, m, wn);
  const cur = new Date(s);
  while (cur <= e) { if (cur.getDay() === 0) return cur; cur.setDate(cur.getDate() + 1); }
  return e;
}
...
function ProgressRing({ current, target, stretch }) {
  ...
  const isStretch = current >= stretch;
  const isTarget = current >= target;
  const remaining = target - current;

  const r = 72, cx = 120, cy = 120, lw = 14;
  const circ = 2 * Math.PI * r;
  const offset = circ * (1 - pct);
  const strokeColor = isStretch ? P.gold : P.nebula;
  const glowColor = isStretch ? P.gold : P.nebula;

  const subText = isStretch
    ? "You reached the far stars."
    : isTarget
    ? "Monthly constellation complete."
    ...
    <div className="ring-container">
      ...
      <svg width="240" height="240" viewBox="0 0 240 240" className="ring-svg">
        <circle cx={cx} cy={cy} r={r + 18} fill="none"
          stroke={glowColor} strokeWidth="1" className="halo-ring" />
        <circle cx={cx} cy={cy} r={r} fill="none"
          stroke={P.dim} strokeWidth={lw} strokeLinecap="round" />
        <circle cx={cx} cy={cy} r={r} fill="none"
          stroke={strokeColor} strokeWidth={lw} strokeLinecap="round"
          strokeDasharray={circ} strokeDashoffset={offset}
          transform={`rotate(-90 ${cx} ${cy})`}
          style={{ transition: "stroke-dashoffset 0.8s ease, stroke 0.5s ease" }} />
        <text x={cx} y={cy - 10} textAnchor="middle" fill={P.text}
          style={{ fontSize: "36px", fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 600 }}>
          {current}
        </text>
        <text x={cx} y={cy + 18} textAnchor="middle" fill={P.soft}
          style={{ fontSize: "13px", fontFamily: "'Inter', 'Helvetica Neue', sans-serif" }}>
          of {target} stars
        </text>
      </svg>
      <p className="ring-sub" style={{ color: isStretch ? P.gold : isTarget ? P.nebula : P.soft }}>
        {isStretch && <span className="sparkle-icon">✦ </span>}
        {subText}
      </p>
    </div>
  );
...
function LogModal({ onClose, onLog }) {
  ...
  const preview = calcPts({
    activity_type: activity, duration_min: duration,
    intensity_flag: activity === "yoga" ? flag : false,
    intentional_flag: activity === "walk" ? flag : false,
  });

  const handleLog = () => {
    onLog({
      date: todayStr(), activity_type: activity, duration_min: duration,
      intensity_flag: activity === "yoga" ? flag : false,
      intentional_flag: activity === "walk" ? flag : false,
    });
    onClose();
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", marginBottom: 20 }}>
          {["yoga", "walk"].map(a => (
            <button key={a} onClick={() => { setActivity(a); setFlag(false); }}
              className={`activity-btn ${activity === a ? "active" : ""}`}
              style={{ "--accent": a === "yoga" ? P.yoga : P.walk }}>
              <span className="activity-star">✦</span> {a.charAt(0).toUpperCase() + a.slice(1)}
            </button>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: "center", marginBottom: 16 }}>
          ...
          <select value={duration} onChange={e => setDuration(Number(e.target.value))} className="duration-select">
            {[15, 20, 25, 30, 35, 40, 45, 50, 60, 75, 90].map(v => (
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content modal-reward" onClick={e => e.stopPropagation()}>
        ...
        {["gold", "silver", "bronze"].map(tier => {
          ...
          const need = tier === "gold" ? 15 : tier === "silver" ? 12 : 9;
          return (
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 20 }}>
          <button className="btn-primary" onClick={() => { onClaim(rk, selected); onClose(); }}>
            ✦ Receive Your Star
          </button>
          <button className="btn-ghost" onClick={onClose}>Cancel</button>
        </div>
...
export default function StarFlow() {
  ...
  const [claimed, setClaimed] = useState(() => loadData().claimed);
  ...
  const delEntry = (idx) => {
    const todayEs = entries.map((e, i) => ({ ...e, _i: i })).filter(e => e.date === todayStr());
    if (idx < todayEs.length) {
      setEntries(prev => prev.filter((_, i) => i !== todayEs[idx]._i));
    }
  };
  const claimReward = (rk, sel) => {
    setClaimed(prev => [...prev, rk]);
    const [tier, reward] = sel.split(":", 2);
    setCelebrate({ tier, reward });
  };
  const unclaim = (rk) => {
    setClaimed(prev => prev.filter(c => c !== rk));
  };
  ...
  const todayPtsVal = dailyPts(entries, today);
  const tier = topTier(wp);
  ...
  if (!tier) { peekText = `${WEEKLY_TARGET - wp} stars from a weekend gift`; }
    ...
    const at = availTiers(wp);
    const h = at[at.length - 1] || "bronze";
    const sample = REWARDS[h][Math.floor(Math.random() * REWARDS[h].length)];
    ...
    <div className="star-flow-app">
      ...
      <div className="app-scroll">
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <button className="btn-primary btn-large" onClick={() => setShowLog(true)}>
            ✦ Add a Moment
          </button>
        </div>
        ...
        <GlassCard className="section-card">
          ...
          <div className="cal-grid">
            ...
            {calendarWeeks(viewYear, viewMonth).flat().map((day, i) => {
              ...
              const es = entriesFor(entries, ds);
              const hasAct = es.some(e => calcPts(e) > 0);
              const isToday = isCurrentMonth && day === now.getDate();
              const yc = es.filter(e => e.activity_type === "yoga" && e.duration_min >= 30).length;
              const wc = es.filter(e => e.activity_type === "walk" && e.duration_min >= 20).length;
              const dp = dailyPts(entries, ds);
              return (
                <div key={ds} className={`cal-cell ${isToday ? "today" : ""} ${hasAct ? "active" : ""}`}>
                  <span className="cal-day">{day}</span>
                  {(yc > 0 || wc > 0) && (
                    <div className="cal-dots">
                      {Array(Math.min(yc, 2)).fill(0).map((_, j) => <span key={`y${j}`} className="dot yoga" />)}
                      {Array(Math.min(wc, 2)).fill(0).map((_, j) => <span key={`w${j}`} className="dot walk" />)}
                    </div>
                  )}
          ...
          <div className="cal-legend">
            <span><span className="dot yoga" /> Yoga</span>
            <span><span className="dot walk" /> Walk</span>
            <span><span className="dot bonus" /> Star bonus</span>
          </div>
          <p style={{ textAlign: "center", color: P.soft, fontSize: 12, marginTop: 4, paddingBottom: 10 }}>
            {stats.yc} yoga ({stats.ys}✦)  ·  {stats.wc} walks ({stats.ws}✦)
          </p>
        </GlassCard>
        ...
        <GlassCard className="section-card">
          ...
          {todayEntries.length === 0 ? (
            ...
            todayEntries.map((entry, i) => {
              const a = entry.activity_type;
              const pts = calcPts(entry);
              const hasFlag = entry.intensity_flag || entry.intentional_flag;
              const accent = a === "yoga" ? P.yoga : P.walk;
              return (
                <div key={i} className="entry-card" style={{ "--accent": accent, background: a === "yoga" ? P.yogaLight : P.walkLight }}>
                  <div className="entry-left">
                    <span style={{ color: accent }}>✦</span>
                    <span className="entry-name">{a.charAt(0).toUpperCase() + a.slice(1)}  ·  {entry.duration_min} min</span>
                    {hasFlag && <span className="bonus-star" style={{ color: P.gold }}>✦</span>}
                  </div>
        ...
        <GlassCard className="section-card">
          <div className="card-header">
            ...
            <span className="card-pts" style={{ color: P.nebula }}>{wp} / {WEEKLY_TARGET}</span>
          </div>
          {streak > 0 ? (
        ...
        <div className="week-pills">
          {Array.from({ length: numWeeks }, (_, i) => i + 1).map(wk => {
            const wkPts = weekPts(entries, viewYear, viewMonth, wk);
            const wkTier = topTier(wkPts);
            const isCur = isCurrentMonth && wk === cw;
            return (
              <GlassCard key={wk} className={`week-pill ${isCur ? "current" : ""}`}
                glow={isCur} glowColor={P.nebula}>
                <span className="wp-label">W{wk}</span>
                <span className="wp-pts">{wkPts}</span>
                {wkTier ? <span className="wp-tier" style={{ color: tierColor(wkTier) }}>✦</span>
        ...
        <div className="section-header" style={{ marginTop: 20 }}>
          <h3 style={{ color: P.text, fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 18 }}>
            ☽ Weekend Stars
          </h3>
        ...
        {Array.from({ length: numWeeks }, (_, i) => i + 1).map(wk => {
          const wkPts = weekPts(entries, viewYear, viewMonth, wk);
          const avail = availTiers(wkPts);
          const tt = topTier(wkPts);
          const rk = `${viewYear}-${String(viewMonth).padStart(2, "0")}-W${wk}`;
          const isClaimed = claimed.includes(rk);
          const fri = fridayOf(viewYear, viewMonth, wk);
          const sun = sundayOf(viewYear, viewMonth, wk);
          const unlocked = new Date() >= fri;
          ...
          return (
            <GlassCard key={rk} className="section-card reward-row">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {!avail.length ? (
                  <span style={{ color: P.dim, fontSize: 12 }}>{WEEKLY_TARGET - wkPts} stars away</span>
                ) : !unlocked ? (
      ...
      {rewardModal && (
        <RewardModal {...rewardModal}
          ...
          onClaim={(rk, sel) => claimReward(rk, sel)} />
      )}
...
.star-flow-app {
  min-height: 100vh;
  background: linear-gradient(180deg, ${P.bgTop} 0%, ${P.bg} 40%, ${P.bgBottom} 100%);
...
.glass-card.glass-glow {
  box-shadow: 0 0 20px color-mix(in srgb, var(--glow-color) 15%, transparent);
...
.cal-arrow {
  ...
  color: ${P.moon}; font-size: 22px; font-weight: 700;
...
.cal-cell.today {
  ...
  border: 1.5px solid ${P.nebula};
}
.cal-cell.active {
  background: rgba(25, 21, 53, 0.5);
  border: 1px solid ${P.dim};
}
.cal-day {
  font-size: 10px;
...
.cal-cell.active .cal-day { color: ${P.text}; font-weight: 600; }
.cal-dots {
  display: flex;
  gap: 2px;
  margin-top: auto;
...
.celebrate-burst {
  font-size: 48px;
  color: ${P.gold};
  animation: spin-burst 1.2s ease;
  margin-bottom: 12px;
}
